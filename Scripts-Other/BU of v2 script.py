...
if org and search_button:
    ...
    # --- 2b) Fuzzy match to Yelp ---
if match is not None:
    cms_name = match.get("Hospital Name")
    cms_city = match.get("City") or default_loc  # fallback to default if city missing

    with st.spinner("Fetching Yelp reviews..."):
        # Try API first
        yelp_reviews = fetch_yelp_reviews_api(cms_name, cms_city, yelp_key)
        if not yelp_reviews:  # fallback to scraper if API fails or returns nothing
            yelp_reviews = fetch_yelp_reviews_scrape(cms_name, cms_city)

    if yelp_reviews:
        st.subheader("Yelp Reviews")
        for review in yelp_reviews:
            st.markdown(f"- **{review.get('user','Anonymous')}** ({review.get('rating','N/A')}/5): {review.get('text','')}")
    else:
        st.info("No Yelp reviews found for this organization.")
